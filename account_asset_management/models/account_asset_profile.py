...
class AccountAssetProfile(models.Model):
    ...
    account_asset_id = fields.Many2one(comodel_name="account.account", domain=[("deprecated", "=", False)],
                                       string="Asset Account", required=True)
    account_depreciation_id = fields.Many2one(comodel_name="account.account", domain=[("deprecated", "=", False)],
                                              string="Depreciation Account", required=True)
    account_expense_depreciation_id = fields.Many2one(comodel_name="account.account", required=True,
                                                      domain=[("deprecated", "=", False)],
                                                      string="Depr. Expense Account")
    account_plus_value_id = fields.Many2one(comodel_name="account.account", domain=[("deprecated", "=", False)],
                                            string="Plus-Value Account", )
    account_min_value_id = fields.Many2one(comodel_name="account.account", domain=[("deprecated", "=", False)],
                                           string="Min-Value Account", )
    account_residual_value_id = fields.Many2one(comodel_name="account.account", domain=[("deprecated", "=", False)],
                                                string="Residual Value Account", )
    journal_id = fields.Many2one(comodel_name="account.journal", domain=[("type", "=", "general")],
                                 string="Journal", required=True)
    company_id = fields.Many2one(comodel_name="res.company", string="Company", required=True,
                                 default=lambda self: self._default_company_id(), )
    group_ids = fields.Many2many(comodel_name="account.asset.group", relation="account_asset_profile_group_rel",
                                 column1="profile_id", column2="group_id", string="Asset Groups", )
    method = fields.Selection(selection=lambda self: self._selection_method(), string="Computation Method",
    ...
    method_period = fields.Selection(selection=lambda self: self._selection_method_period(),
    ...
    method_time = fields.Selection(selection=lambda self: self._selection_method_time(),
    ...
    def _selection_method(self):
        ...
        return [("linear", _("Linear")), ("linear-limit", _("Linear up to Salvage Value")),
                ("degressive", _("Degressive")), ("degr-limit", _("Degressive  up to Salvage Value")), ]
    ...
    def _check_method(self):
        for profile in self:
            if profile.method == "degr-linear" and profile.method_time != "year":
                raise UserError(_("Degressive-Linear is only supported for Time Method = " "Year."))
