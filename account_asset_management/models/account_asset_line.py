...
class AccountAssetLine(models.Model):
    ...
    asset_id = fields.Many2one(comodel_name="account.asset", string="Asset", required=True, ondelete="cascade")
    previous_id = fields.Many2one(comodel_name="account.asset.line", string="Previous Depreciation Line",
                                  readonly=True, )
    parent_state = fields.Selection(related="asset_id.state", string="State of Asset", readonly=True)
    depreciation_base = fields.Monetary(related="asset_id.depreciation_base", string="Depreciation Base", readonly=True)
    amount = fields.Monetary(string="Amount", digits="Account", required=True)
    remaining_value = fields.Monetary(compute="_compute_values", digits="Account", string="Next Period Depreciation",
                                      store=True, )
    depreciated_value = fields.Monetary(compute="_compute_values", digits="Account",
    ...
    move_id = fields.Many2one(comodel_name="account.move", string="Depreciation Entry", readonly=True)
    move_check = fields.Boolean(compute="_compute_move_check", string="Posted", store=True)
    ...
    def _onchange_amount(self):
        if self.type == "depreciate":
            self.remaining_value = (self.depreciation_base - self.depreciated_value - self.amount)
    ...
    def write(self, vals):
        for dl in self:
            ...
            if list(vals.keys()) == ["move_id"] and not vals["move_id"]:
                ...
                raise UserError(_("You cannot change a depreciation line "
    ...
    def unlink(self):
        ...
        return super(AccountAssetLine, self.with_context(no_compute_asset_line_ids=self.ids)).unlink()
    ...
    def create_move(self):
        ...
        for line in self:
            ...
            aml_d_vals = line._setup_move_line_data(depreciation_date, depr_acc, "depreciation", move)
            self.env["account.move.line"].with_context(ctx).create(aml_d_vals)
            aml_e_vals = line._setup_move_line_data(depreciation_date, exp_acc, "expense", move)
