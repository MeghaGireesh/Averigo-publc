...
#                 asset = (
#                     self.env["account.asset"].with_context(create_asset_from_move_line=True, move_id=move.id).create(
#                         vals))
...
#         self.account_id = self.asset_profile_id.account_asset_id.id
...
#                     raise UserError(_("You are not allowed to link "
...
#                 self.env.context.get("allow_asset_removal") and list(vals.keys()) == ["asset_id"]):
...
#                 if linked_asset:
#                     raise UserError(_("You cannot change an accounting item "
#                                       "linked to an asset depreciation line."))

#         if (self.filtered(lambda r: not r.move_id.is_sale_document())
#                 and vals.get("asset_id") and not self.env.context.get("allow_asset")):
#             raise UserError(_("You are not allowed to link "
#                               "an accounting entry to an asset."
#                               "\nYou should generate such entries from the asset."))
#         super().write(vals)
