...
class AccountAssetRemove(models.TransientModel):
    ...
    date_remove = fields.Date(string="Asset Removal Date", required=True, default=fields.Date.today,
    ...
    sale_value = fields.Float(string="Sale Value", default=lambda self: self._default_sale_value())
    account_sale_id = fields.Many2one(comodel_name="account.account", string="Asset Sale Account",
                                      domain=[("deprecated", "=", False)],
                                      default=lambda self: self._default_account_sale_id(), )
    account_plus_value_id = fields.Many2one(comodel_name="account.account", string="Plus-Value Account",
                                            domain=[("deprecated", "=", False)],
                                            default=lambda self: self._default_account_plus_value_id(), )
    account_min_value_id = fields.Many2one(comodel_name="account.account", string="Min-Value Account",
                                           domain=[("deprecated", "=", False)],
                                           default=lambda self: self._default_account_min_value_id(), )
    account_residual_value_id = fields.Many2one(comodel_name="account.account", string="Residual Value Account",
                                                domain=[("deprecated", "=", False)],
                                                default=lambda self: self._default_account_residual_value_id(), )
    posting_regime = fields.Selection(selection=lambda self: self._selection_posting_regime(),
    ...
    def _get_sale(self):
        ...
        inv_lines = self.env["account.move.line"].search(
            [("asset_id", "=", asset_id), ("move_id.type", "in", ("out_invoice", "out_refund")), ])
        for line in inv_lines:
            inv = line.move_id
            comp_curr = inv.company_currency_id
            inv_curr = inv.currency_id
            if (line.move_id.invoice_payment_state == "paid" or line.parent_state == "draft"):
    ...
    def _selection_posting_regime(self):
        return [("residual_value", _("Residual Value")), ("gain_loss_on_sale", _("Gain/Loss on Sale")), ]
    ...
    def remove(self):
        ...
        asset_id = self.env.context.get("active_id")
        asset = self.env["account.asset"].browse(asset_id)
        asset_ref = (asset.code and "{} (ref: {})".format(asset.name, asset.code) or asset.name)
        ...
        dlines = asset_line_obj.search([("asset_id", "=", asset.id), ("type", "=", "depreciate")],
                                       order="line_date desc", )
        if dlines:
            last_date = dlines[0].line_date
        else:
            print("aasset id",asset.id,asset_line_obj.search([("asset_id", "=", asset.id), ("type", "=", "create")]))
            create_dl = asset_line_obj.search([("asset_id", "=", asset.id), ("type", "=", "create")])[0]
            last_date = create_dl.line_date

        if self.date_remove < last_date:
            raise UserError(_("The removal date must be after " "the last depreciation date."))
    ...
    def _prepare_early_removal(self, asset):
        ...
        def _dlines(asset):
            ...
            dlines = lines.filtered(lambda l: l.type == "depreciate" and not l.init_entry and not l.move_check)
        ...
        if date_remove > first_date:
            raise UserError(_("You can't make an early removal if all the depreciation "
        ...
        if first_to_depreciate_dl.previous_id:
            ...
            create_dl = asset_line_obj.search([("asset_id", "=", asset.id), ("type", "=", "create")])
    ...
    def _get_removal_data(self, asset, residual_value):
        ...
        if residual_value:
            if self.posting_regime == "residual_value":
                ...
                account_id = (self.account_plus_value_id.id if balance > 0 else self.account_min_value_id.id)
                move_line_vals = {
                    "name": asset.name,
